...
function sign_up($email, $voornaam, $achternaam, $password, $repassword, $ip){

    require_once('class/DBConfig.php');

    if($email != '' && $voornaam != '' && $achternaam != '' && $password != '' && $repassword != '' && $ip != ''){
        ...
        if($result->num_rows < 3){
            ...
            if($result->num_rows == 0){
                if($password == $repassword){
                    $sql 	= "INSERT INTO `users` (`email`, `voornaam`, `achternaam`, `password`, `lang`, `datum`, `ip`) VALUES ('".save_mysql($email)."', '".save_mysql($voornaam)."', '".save_mysql($achternaam)."', '".md5(save_mysql($password))."', 'EN', '".date("d/m/Y")."', '".save_mysql($ip)."');";
                    $conn = connection();
...
function getFirstName($id)
{
    ...
    $sql 	= "SELECT `voornaam` FROM `users` WHERE `id`='".$id."'";
    $result = connection()->query($sql);
    if($result->num_rows > 0)
    {
        return $result->fetch_assoc()['voornaam'];
    }
    else
    {
        return "";
...
function getFamilyName($id)
{
    ...
    $sql 	= "SELECT `achternaam` FROM `users` WHERE `id`='".$id."'";
    $result = connection()->query($sql);
    if($result->num_rows > 0)
    {
        return $result->fetch_assoc()['achternaam'];
    }
    else
    {
        return "";
