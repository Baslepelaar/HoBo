		...
		if(!empty($_GET['park_id'])) {
			if(canManageparkRequests($_SESSION['id'])) {
				if(isset($_REQUEST['accept'])) {
					acceptParkRequest($park_id);
					header('Location: staff-panel.php?success=You succesfully <strong>accepted</strong> the park request.');
					exit;
				}
				if(isset($_REQUEST['reject'])) {
					rejectParkRequest($park_id);
					header('Location: staff-panel.php?success=You succesfully <strong>rejected</strong> the park request.');
					exit;
				}
			}
			else {
				header('Location: staff-panel.php?danger=You are not allowed to manage the park requests. If this is wrong you can contact a site administrator.');
			}
		}
?>
<!DOCTYPE html>
<html>
	<head>
		...
		<title>MineThemepark | Staff Panel</title>
		<style type="text/css">
		body {
